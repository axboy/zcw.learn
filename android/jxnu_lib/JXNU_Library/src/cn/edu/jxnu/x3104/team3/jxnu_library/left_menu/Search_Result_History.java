...
import cn.edu.jxnu.x3104.team3.jxnu_library.ui.Search_detail;
import cn.edu.jxnu.x3104.team3.jxnu_library.ui.Search_result_no_content;

@SuppressLint("HandlerLeak")
public class Search_Result_History extends Activity{
	TextView back;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		back.setOnClickListener(new View.OnClickListener(){
			...
			public void onClick(View v) {
				Search_Result_History.this.finish();
		...
		mainhandler=new  Handler(){
			public void handleMessage(Message msg){
				if(msg.what==0x200)
				{
					...
					intent.setClass(Search_Result_History.this,Search_detail.class);
		...
		new Thread(new Runnable(){
			public void run() {
				...
				String history_url=intent.getStringExtra("history_url");
				String result =new DoConnection().gethtmlcode(Search_Result_History.this, history_url);
				if(result.contains("本馆没有您检索的馆藏书目")){
					new Thread(new Runnable(){
						public void run(){
							Intent intent=new Intent();
							intent.putExtra("title", "搜索记录");
							intent.setClass(Search_Result_History.this,Search_result_no_content.class);
							startActivity(intent);
							Search_Result_History.this.finish();
						}
					}).start();
